...
def is_parallel(model):
    """返回bool 判断是否使用的是GPU并行方式1 DDP or 使用并行方式2 DP"""
    return isinstance(model, (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel)) # isinstance判断一个对象是否是一个已知的类型

# model.module主要用于在模型并行化处理后获取原始模型，以便进行如保存模型、加载模型等操作。
def de_parallel(model):
    """返回sequential容器封装的模型 去并行化模型：如果模型是DP或DDP类型，则返回单个GPU模型。
       所谓去并行化，意思是从并行化后model中取出模型
    """
