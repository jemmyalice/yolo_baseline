    ...
    def forward(self, im, augment=False, visualize=False, embed=None):
        ...
        if isinstance(im, list):
            ...
            if self.fp16 and im[0].dtype != torch.float16:
                im[0] = im[0].half()  # to FP16
                im[1] = im[1].half()  # to FP16
            if self.nhwc:
                im[0] = im[0].permute(0, 2, 3, 1)  # torch BCHW to numpy BHWC shape(1,320,192,3)
                im[1] = im[1].permute(0, 2, 3, 1)  # torch BCHW to numpy BHWC shape(1,320,192,3)
            # PyTorch
