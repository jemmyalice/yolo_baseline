...
class MF12(nn.Module):  # stereo attention block
    def __init__(self, channels):
        super(MF12, self).__init__()
    ...
    def init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                if m.bias is not None:
                    init.constant_(m.bias, 0)
    def forward(self, x, y):  # B * C * H * W #x_   left, x_right right为ir，left为rgb
        ...
        out_RGB, out_IR = self.cmd(x_mask_left + x_left_ori, x_mask_right + x_right_ori)

        out_IR = self.bottleneck1(out_IR)
        out_RGB = self.bottleneck2(out_RGB)  # RGB
