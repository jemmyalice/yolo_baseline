...
class ECAAttention1(nn.Module):
    def __init__(self, ch_in, kernel_size = 2):
        super().__init__()
        self.gap = nn.AdaptiveAvgPool2d(1)
        ...
        self.conv1 = nn.Conv1d(1, 1, kernel_size=kernel_size, padding=1)
    ...
    def forward(self, x):
        ...
        y1 = self.gap1(x)  # 在空间方向执行全局平均池化: (B,C,H,W)-->(B,C,1,1)
        y1 = y1.squeeze(-1).permute(0, 2, 1)  # 将通道描述符去掉一维,便于在通道上执行卷积操作:(B,C,1,1)-->(B,C,1)-->(B,1,C)
        y1 = self.conv1(y1)  # 在通道维度上执行1D卷积操作,建模局部通道之间的相关性: (B,1,C)-->(B,1,C)
