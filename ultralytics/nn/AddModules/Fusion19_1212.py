...
class ECAAttention1(nn.Module):
    def __init__(self, ch_in, kernel_size = 3, kernel_size1 = 1):
        ...
        self.weight1 = nn.Parameter(torch.tensor(1.0))  # 对应于 y1 的权重
        self.weight2 = nn.Parameter(torch.tensor(1.0))  # 对应于 y1 的权重
    ...
    def forward(self, x):
        ...
        y = y * self.weight1 + y1 * self.weight2
        y = self.sigmoid(y)  # 生成权重表示: (B,1,C)

        return x * y.expand_as(x) # 权重对输入的通道进行重新加权: (B,C,H,W) * (B,C,1,1) = (B,C,H,W)

class MF_19(nn.Module):  # stereo attention block
    def __init__(self, channels):
        super(MF_19, self).__init__()
