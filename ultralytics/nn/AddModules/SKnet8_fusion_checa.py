...
class ECAAttention1(nn.Module):
    ...
    def forward(self, x):
        ...
        return (x*y.expand_as(x) + x*y1.expand_as(x))  # 权重对输入的通道进行重新加权: (B,C,H,W) * (B,C,1,1) = (B,C,H,W)
...
class MF10(nn.Module):  # stereo attention block
    def __init__(self, channels):
        super(MF10, self).__init__()
    ...
    def forward(self, x, y):  # B * C * H * W #x_   left, x_right
        ...
        x_left = self.se_r(x_left_ori) * 0.5
        x_right = self.se_i(x_right_ori) * 0.5
        x_left = x_left * 0.5
        x_right = x_right * 0.5
