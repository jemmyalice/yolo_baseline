...
# 添加一个 1x1 卷积 (self.cv2) 来实现更复杂的卷积结构，同时支持卷积融合（fusing），以提高推理速度。
class Conv2(Conv):
    ...
    def fuse_convs(self):
        ...
        self.__delattr__("cv2") # 删除融合后的cv2
        self.forward = self.forward_fuse # 推理阶段只使用融合后的卷积层
...
class DWConv(Conv):
    ...
    def __init__(self, c1, c2, k=1, s=1, d=1, act=True):  # ch_in, ch_out, kernel, stride, dilation, activation
        ...
        # groups=math.gcd(c1, c2)，这意味着 groups 参数会根据输入通道数 c1 和输出通道数 c2 的最大公约数来确定。
