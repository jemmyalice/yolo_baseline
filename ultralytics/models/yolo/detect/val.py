...
from ultralytics.utils.metrics import ConfusionMatrix, DetMetrics, box_iou # 在YOLOv5中，fitness函数实现对 [P, R, mAP@.5, mAP@.5-.95] 指标进行加权
from ultralytics.utils.plotting import output_to_target, plot_images  # 定义了Annotator类，可以在图像上绘制矩形框和标注信息


class DetectionValidator(BaseValidator):
    """
    一个扩展BaseValidator类的类，用于基于检测模型进行验证。
    ...
    def __init__(self, dataloader=None, save_dir=None, pbar=None, args=None, _callbacks=None, infusion=True):
        """Initialize detection model with necessary variables and settings."""
        super().__init__(dataloader, save_dir, pbar, args, _callbacks, infusion)
